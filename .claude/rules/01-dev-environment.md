...
The native SwiftUI app has its own coverage lane: [`ios-tests.yml`](../../.github/workflows/ios-tests.yml) runs the `omnibusTests` suite on a simulator via [`scripts/ios-test.sh`](../../scripts/ios-test.sh) (a dedicated `omnibus-tests` device it creates on the newest installed iPhone runtime and pre-boots, path-gated on `omnibus-ios/**` with the same skipped-counts-as-pass `ci-gate` pattern as rust.yml), converts the `.xcresult` archive to lcov with [`scripts/xccov-to-lcov.sh`](../../scripts/xccov-to-lcov.sh), and uploads it under the `ios` flag in `codecov.yml` — report-only, like the Rust lanes.

**Diagnosing an XCUITest failure that only happens in CI.** An XCUITest assertion message reaches neither xcodebuild's stdout nor the JUnit report — JUnit records a bare `Parallel test failed` — so a red `UI tests (omnibusUITests)` job tells you *which* test failed and nothing else. The message lives only inside the `.xcresult`, which the `ui` job uploads as the `ui-xcresult` artifact on failure; download it and read it back with `xcrun xcresulttool get test-results tests --path ui.xcresult`. The same applies locally: `scripts/ios-test.sh` leaves the bundle at `.claude/runtime/ios-tests/<suite>.xcresult`, and that — not the terminal output — is where the assertion is.
