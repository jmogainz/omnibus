...
Omnibus is a self-hosted book library for personal use — the ebook and audiobook equivalent of Plex/Jellyfin. It scans a local directory of ebooks (epub) and audiobooks (m4a), organizes them into configurable libraries filtered by metadata, and lets authenticated users browse, read, listen, journal, and rate books. It aims to go beyond what another implementation and AudioBookShelf offer, with first-class device sync, in-browser reading and listening, cross-device progress tracking, and a native mobile app.
