...
Supersedes the prior `ROADMAP.md` (deleted alongside this document's introduction). Synthesizes the v1 feature list with [another implementation-analysis.md](another implementation-analysis.md), reorganized to reflect the foundational work v1 assumed but did not sequence, and to incorporate performance lessons learned from another implementation.

---

## 1. Vision

Omnibus is a self-hosted book library for personal use — the ebook and audiobook equivalent of Plex/Jellyfin. It scans local directories of ebooks (epub) and audiobooks (m4a/m4b), organizes them into user-configurable libraries filtered by metadata, and lets authenticated users browse, read, listen, journal, and rate books. It aims to go beyond what another implementation and AudioBookShelf offer, with first-class device sync (Kobo, Kindle), in-browser reading and listening, cross-device progress tracking, and a native mobile app built from the same Rust codebase.
...
another implementation analysis reveals three classes of issue we can avoid if we act early:

- **Schema foundation.** another implementation's performance PRs were retrofits against a denormalized schema. We are about to make the same mistake: our current `books` table is one row per file with JSON-blob relationships. Fixing this once, early, is cheaper than any later N+1 optimization.
- **Search.** another implementation shipped SQLite FTS5 as an opt-in fast path that only activates if Calibre-desktop created the index. We can make it the default.
- **Sequencing.** another implementation's single `WorkerThread` is a performance ceiling it cannot lift without a rewrite. We have the stack to parallelize from day one — but only if we build the worker primitive before the first async feature needs it.
...
- Vision is crisp: "Plex/Jellyfin for books, beating another implementation."
- Feature list is comprehensive and well-specified per feature (acceptance criteria, implementation plan).
- Stack choices (axum, sqlx, Dioxus fullstack + native) are correct and synergistic — see another implementation-analysis §4.
...
| # | Gap | Impact |
|---|---|---|
| G1 | **No search feature.** Browse-by-filter ≠ search. another implementation users constantly hit this. | Core UX miss; users can't find a book they remember by title fragment. |
...
| G7 | **No metadata editing.** another implementation's most-used admin surface is absent. | Users with messy libraries can't fix anything without hand-editing files. |
| G8 | **No observability.** No logging, metrics, or admin log viewer spec. | First production bug will be invisible. |
| G9 | **Kobo via OPDS only.** another implementation's native Kobo-sync endpoint (`/kobo/v1/*`) is a closer UX than OPDS and isn't mentioned. | Leaves the best-supported Kobo integration off the table. |
...
- **Upload target:** v1 requires writing files into a scan path; but [another implementation-analysis.md §7](another implementation-analysis.md#7-recommendations-for-omnibus) argues against the DB mutating the filesystem for conflict-avoidance. These can coexist (uploads ≠ edits) but the distinction needs to be explicit.
...
- **Tech.** Mirror Calibre's `data` table layout for path compatibility (see another implementation-analysis §5). Add indices on every m2m reverse column and on `books.uuid`, `books.last_modified`, `books.sort`, `books.series_index`. `COLLATE NOCASE` on searchable strings.
...
- **Tech.** `argon2` + `tower-sessions` backed by SQLite. `auth_required` / `admin_required` axum extractors. Explicit permission columns — **do not** copy another implementation's role bitmask (see analysis §7 rec #9).
...
- **Scope.**
  ...
  - **No renaming on metadata edit.** Metadata overrides live in the DB (`books.metadata_overrides` JSON), never mutate folder names or file contents. Explicitly rejects another implementation's folder-rename-on-edit path (racy with readers and scanners).
...
- **Value.** Avoids another implementation's single-`WorkerThread` ceiling. Keeps web path responsive while CPU-bound work runs.
...
- **Value.** The feature another implementation users most complain about. Cheap to ship on top of F0.4.
...
- **Value.** Cover grids are the highest-bandwidth path. WebP at 3 sizes delivers ~30% smaller payloads than another implementation's scheduled JPEG pipeline.
...
- **Changes from v1.** Pagination via keyset (not `OFFSET`) — another implementation-analysis §3 shows `OFFSET` is the large-library killer. Sort happens client-side on already-hydrated lists for ≤10k-book libraries (leverage Dioxus signals; see analysis §7 rec #12).
...
- **Objective.** Implement `/kobo/v1/library/sync`, `/state`, `/metadata`, `/tags`, `download/*` endpoints (see another implementation-analysis §6).
- **Value.** Far superior to OPDS on Kobo: background sync, reading-state round-trip, shelves-as-tags, no manual re-browse.
- **Tech.** Stream the sync response via `axum::body::StreamBody` — **do not** copy another implementation's 100-item cap (analysis §3, §7 rec #13).
...
- **Risks.** Undocumented Kobo protocol; another implementation's implementation is our reference. Scope to "parity with another implementation minus 100-item cap" for v1.0.
...
- **Value.** Self-hosted libraries are messy; the ability to fix metadata without shelling into the server is table stakes. Absence of this is another implementation's biggest retention hook.
...
- **Value.** First production bug is free of guesswork; scheduled-task visibility is a another implementation feature we should not regress.
...
- **Value.** Gives power users the ability to generate AZW3, PDF, TXT, FB2, etc. without leaving Omnibus. Closes a real another implementation parity gap for the subset of users who actually use it.
- **Scope cap.**
...
- **Phase 0 delays visible progress by 4–6 weeks.** Accepted — every week spent on Phase 0 saves multiple weeks of rework later (see another implementation PR #3476 as evidence of the cost of the alternative).
- **Custom metadata via EAV/JSON, not per-column tables.** Trades some query flexibility for drastic schema simplicity vs. another implementation's runtime-generated DDL.
- **FTS5 triggers add ~10% write amplification** on indexing. Acceptable; reads dominate.
- **Keyset pagination complicates "jump to page N" UX.** another implementation has this UX but it's also the user-reported slow path. Swap for infinite-scroll + search.
