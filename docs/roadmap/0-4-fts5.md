...
bm25-ranked, tokenized, diacritic-insensitive search. No `LIKE '%q%'` anywhere. another implementation shipped FTS5 as an opt-in fast path that only activates when Calibre-desktop created the virtual table (see [calibre-inspection §2](../calibre-inspection/2-feature-inventory.md)) — we make the fast path the only path.
