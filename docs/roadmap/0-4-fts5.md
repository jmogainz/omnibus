...
bm25-ranked, tokenized, diacritic-insensitive search across title / authors / series / tags / description / isbn. No `LIKE '%q%'` anywhere. another implementation shipped FTS5 as an opt-in fast path that only activated when Calibre-desktop created the virtual table (see [calibre-inspection §2](../calibre-inspection/2-feature-inventory.md)) — we make the fast path the only path.
...
- `unicode61 remove_diacritics 2` matches the another implementation default.
...
`db/migrations/0005_fts5.sql` does, in order:
...
- [F0.2 Migrations](0-2-migrations.md) — shipped. Trigger DDL ships as migration `0005_fts5.sql`.
