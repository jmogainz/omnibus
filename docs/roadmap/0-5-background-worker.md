...
Avoids another implementation's single-`WorkerThread` ceiling (see [performance pain points](../calibre-inspection/3-performance-pain-points.md)), where a slow `ebook-convert` blocks every other task. Keeps the web path responsive while CPU-bound work runs.
