...
"I can read this right now without downloading" is a core self-hosted library promise. another implementation has it; we need parity.

## Technical considerations

- `epub.js` is the de-facto library (another implementation also uses it — see [calibre-inspection §2](../calibre-inspection/2-feature-inventory.md)). Vendor or pin via npm — do not CDN-load.
...
- epub.js rendering quirks on complex EPUB 3 layouts. Mitigation: accept parity with another implementation's known-good reader behavior for v1.0.
