...
Closes [gap G8](0-0-summary.md#gaps). First production bug is free of guesswork; scheduled-task visibility is a another implementation feature we should not regress on — their admin surface shows task status, which is the one thing operators actually check.
