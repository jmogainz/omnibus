...
This sidesteps another implementation's scheduled-only pipeline (see [calibre-inspection §3](../calibre-inspection/3-performance-pain-points.md)), where users browsing a freshly imported library before the nightly task runs get the full-size original rescaled by the browser — the common "slow covers" complaint.
...
- **Re-examine [F0.6](0-6-library-filesystem.md) sidecar lookup cost while we have a perf harness.** [`sidecar_cover_for`](../../db/src/library_layout.rs) currently does up to 2× `read_dir` per epub (per-stem then folder-level). Acceptable for canonical Omnibus layouts (1 epub per folder), but may show up on flat-dump libraries with thousands of files in one folder. F1.2 is the natural moment to measure: thumbnail generation iterates every book and stresses the same code path. If measurement shows it matters, swap in a fast-path that probes the expected lowercase names directly via `is_file()` before falling back to the case-insensitive `read_dir`. Tracked in [omnibus#52](https://github.com/seamus-sloan/omnibus/issues/52).
  - **Decision (#52, shipped ahead of the harness):** the direct-path `is_file()` fast path is now in `sidecar_cover_for`. Lookup order is direct-per-stem → ci-per-stem → direct-folder → ci-folder, so on canonical (lowercase) layouts the cover resolves via a few `stat` calls and `read_dir` is skipped entirely; the case-insensitive `read_dir` scans remain as fallbacks, so the matching contract is unchanged. This shipped as a low-risk, correctness-preserving reduction in syscalls — it is strictly fewer syscalls on the common case and identical behavior otherwise. **Still pending:** a flame-graph / benchmark validation on a flat-dump library to confirm the `read_dir` cost actually mattered. Run that once F1.2's perf harness exists; if it shows no measurable win, the fast path is harmless, but the harness will tell us whether further work (e.g. a single cached `read_dir` per folder shared across books) is warranted.
