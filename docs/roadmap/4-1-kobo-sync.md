...
Closes [gap G9](0-0-summary.md#gaps). Native Kobo sync is a **far superior UX** to OPDS on Kobo devices: background sync, reading-state round-trip, shelves-as-tags, no manual re-browse. another implementation's implementation is the de-facto reference — users arriving from another implementation expect this as table stakes. Promoted ahead of OPDS in the phasing because it's the better UX for the platform that matters most.
...
- **Stream the sync response** via `axum::body::StreamBody` — **do not** copy another implementation's `SYNC_ITEM_LIMIT=100` cap. See [calibre-inspection §3](../calibre-inspection/3-performance-pain-points.md) and [recommendation #13](../calibre-inspection/7-recommendations.md).
...
- Kobo protocol is undocumented. another implementation's implementation is our reference; scope for v1.0 is "parity with another implementation minus the 100-item cap."
- kepubify absence degrades gracefully; not a blocker.
- **⚠️ Annotation data-loss hazard.** A Kobo expects an annotation channel when syncing (it has one against Kobo's own cloud). Pointing the device at a self-hosted server that doesn't speak that channel can make the Kobo **delete its own local highlights/notes** ([#2610](https://github.com/another-project/another-implementation/issues/2610), [#1783](https://github.com/another-project/another-implementation/issues/1783)). Mitigations: warn the user that first sync may clear on-device annotations, and recommend running the [F4.4 USB import](4-4-kobo-annotation-import.md) **before** the device's first wireless sync. Never claim to preserve device annotations we don't store.
...
- **Scope: per-shelf opt-in — never whole-library.** Mark specific
  ...
  single-book shelf covers the "just one book" case. This matches another implementation's
...
- **Device auth token: persistent, re-viewable URL.** A per-user token in the URL
  ...
  in `.kobo/Kobo/Kobo eReader.conf`. Stored re-viewably (another implementation style) with a
...
- **Endpoint contract tests** replay the device sequence (`initialization →
  ...
  field-for-field from a pinned another implementation revision. A real-device smoke test
