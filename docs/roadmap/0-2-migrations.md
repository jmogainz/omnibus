...
Lets every subsequent change ship without risking production DBs. Avoids another implementation's runtime `ALTER TABLE ADD COLUMN`-on-OperationalError approach (see [calibre-inspection §5](../calibre-inspection/5-schema-details.md)) — that's fine for a hobbyist project but collapses under schema rewrites.
