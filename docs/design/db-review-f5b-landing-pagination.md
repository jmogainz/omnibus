...
biggest perceived-speed win over another implementation"), but it hard-couples "first
...
2. **Cursor encoding.** Opaque base64-of-`(last_sort, last_id)` vs. raw query
   ...
   (`1-3-library-views.md`: "never `OFFSET`"; another implementation's offset paging is the
